...
Production defaults to `guarded_autopilot`; the other modes are `observe`, `recommend`, `approval`, and `full_autopilot`. Every mutation is a typed plan evaluated against tenant, environment, namespace, workload, role, action, risk, confidence, blast radius, cooldown, concurrency, maintenance-window, replica, reversibility, and verification constraints. Exact managed-pod restarts use the dedicated `restart_pod` action and are verified by confirming the old pod disappears and the controller restores the previous ready-replica count.
