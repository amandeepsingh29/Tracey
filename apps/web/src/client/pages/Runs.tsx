...
export function RunDetailPage() {
  ...
  if (conversationId) {
    ...
    if (graphQuery.error || !graphQuery.data) return <div className="page"><button className="back-link" onClick={() => router.push("/runs")}><ArrowLeft size={15} />Runs</button><ErrorState error={graphQuery.error ?? new Error("Execution graph not found")} onRetry={() => void graphQuery.refetch()} /></div>;
    const graph = graphQuery.data;
    const prompt = graph.nodes.find(({ kind }) => kind === "prompt")?.summary;
    return <div className="page execution-detail-page"><button className="back-link" onClick={() => router.push("/runs")}><ArrowLeft size={15} />Runs</button><PageHeader eyebrow={`AGENT CONVERSATION · TURN ${graph.turnIndex}`} title={prompt ?? `Conversation turn ${graph.turnIndex}`} description={`This is what the prompt caused, in order—from model work to commands, tool results, failures, and the final response. ${graph.model ? `Model: ${graph.model}.` : ""}`} actions={<Link className="button button-secondary" href={`/investigations?new=true&prompt=${encodeURIComponent(`Investigate conversation ${graph.conversationId}, turn ${graph.turnIndex}, from ${start} to ${end}.`)}`}><ShieldCheck size={16} />Ask Tracey about this</Link>} />
